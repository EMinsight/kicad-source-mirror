...
public:
    ...
    bool OpenDocument( const wxString& aPath,
...
BOOST_FIXTURE_TEST_CASE( ServerStartsAndResponds, API_SERVER_E2E_FIXTURE )
{
    ...
    BOOST_CHECK_MESSAGE( Client().GetVersion(),
                         "GetVersion failed: " + Client().LastError() );
}
...
BOOST_FIXTURE_TEST_CASE( OpenSingleBoard, API_SERVER_E2E_FIXTURE )
{
    ...
    BOOST_REQUIRE_MESSAGE( Client().GetItemsCount( document, kiapi::common::types::KOT_PCB_FOOTPRINT,
                                                   &footprintCount ),
